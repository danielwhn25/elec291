...
void main(void)
{
    ...
    // NOTE: If your inductor reads too high (e.g. 1600uH instead of 900uH), 
    // it's because of invisible breadboard capacitance. 
    // Change this stray value to ~0.38e-9 to calibrate it down!
    float c_stray = 0.0e-9;    
    
    float c_total = c_physical + c_stray; 
    // The 0.979k resistor handles the DC biasing, it does not factor into the LC frequency math!
