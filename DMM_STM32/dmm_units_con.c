...
void Configure_Pins (void)
{
    ...
    // 6. Continuity Probe (PB5 / Pin 28) - Input with Pull-up
    GPIOB->MODER &= ~(BIT10 | BIT11); 
    GPIOB->PUPDR |= BIT10; 

    // 7. Speaker Output (PB6 / Pin 29) - Output
...
// Simple blocking beep for continuity
void PlayBeep(int duration_ms)
{
    int i;
    for(i = 0; i < duration_ms * 5; i++) // Approx 2.5kHz tone
    {
        GPIOB->ODR |= BIT6;
        wait_us(200);
        GPIOB->ODR &= ~BIT6;
...
void main(void)
{
    ...
    while(1)
    {
        // --- CONTINUITY CHECK (High Priority) ---
        // If Pin 28 (PB5) is connected to Ground
        if ((GPIOB->IDR & BIT5) == 0) {
            PlayBeep(50); // Short beep per loop iteration
        }
